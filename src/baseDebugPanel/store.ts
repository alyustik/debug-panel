...
export function createPanelStore(): PanelStore {
  ...
  function registerNodes(nodes: Node[], values: Record<Path, unknown>, claimRoot: Path) {
    store.setState((state) => {
      ...
      const previousDescendants = state.nodes[claimRoot] ? collectDescendants(claimRoot, state.nodes) : [];
      const stalePaths = previousDescendants.filter((p) => p !== claimRoot && !incomingPaths.has(p));
      ...
      const nextRoot = state.rootChildren.includes(claimRoot) ? state.rootChildren : [...state.rootChildren, claimRoot];
  ...
  function syncValues(entries: Record<Path, unknown>) {
    store.setState((state) => {
      const updates = Object.entries(entries).filter(([path, value]) => state.values[path] !== value);
